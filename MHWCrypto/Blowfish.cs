/*
This code is a rework of the fantastic work done by AsterixAmpersand.
I removed what was unnecessary to decryption, optimized code for very low allocation and copy, and parallelized the algorithm.
See the original code at https://github.com/AsteriskAmpersand/MHW-Save-Editor
In "MHW Save Editor/src/Crypto" and "MHW Save Editor/src/Utility"
*/
...
namespace MHWCrypto
{
    public class Blowfish
    {
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            for (int i = 0; i < 18; i++)
            {
                uint d = (uint)(((cipherKey[j % cipherKey.Length] * 256 + cipherKey[(j + 1) % cipherKey.Length]) * 256 + cipherKey[(j + 2) % cipherKey.Length]) * 256 + cipherKey[(j + 3) % cipherKey.Length]);
                bf_P[i] ^= d;
            ...
            {
                Encipher(ref xl_par, ref xr_par);
                bf_P[i] = xl_par;
                bf_P[i + 1] = xr_par;
            }
        ...
        private void BlockDecrypt(byte[] block, int index)
        {
            SetBlock(block, index, out uint xl_par, out uint xr_par);
            Decipher(ref xl_par, ref xr_par);
            GetBlock(block, index, xl_par, xr_par);
        }

        private static void InPlaceBitConverterGetBytes(uint value, byte[] buffer, int index)
        {
        ...
        private static uint BitConverterToUInt32(byte[] buffer, int index)
        {
            uint result =
                buffer[index + 0] |
        ...
        private void SetBlock(byte[] block, int index, out uint xl_par, out uint xr_par)
        {
            xl_par = BitConverterToUInt32(block, index + 0);
            xr_par = BitConverterToUInt32(block, index + 4);
        }

        private void GetBlock(byte[] block, int index, uint xl_par, uint xr_par)
        {
            InPlaceBitConverterGetBytes(xl_par, block, index + 0);
            InPlaceBitConverterGetBytes(xr_par, block, index + 4);
        }

        private void Encipher(ref uint xl_par, ref uint xr_par)
        {
            xl_par ^= bf_P[0];
        ...
        private void Decipher(ref uint xl_par, ref uint xr_par)
        {
            xl_par ^= bf_P[17];
